...
def checks(trackstatus):
    ...
    if tracking:
        ...
        trak.core_methods.write_trackdata()
...
def main():
    ...
    if args.record:
        r_list = [['Total number of words tracked', trak.core_methods.n_words()],
                  ['Total number of sessions tracked', trak.core_methods.n_sessions()],
                  ['Total number of modified files', trak.core_methods.n_files()]]
        l_session = Session(trak.core_methods.longest_session())
        ...
        w_session = Session(trak.core_methods.most_words())
        ...
        r_list.append(['Longest writing streak', trak.core_methods.longest_streak()])
        r_list.append(['Current streak', '{} days'.format(trak.core_methods.current_streak())])
    ...
    if args.status:
        ...
        if os.path.exists(tmp_path):
            ...
            MSG = 'Tracking is ON\n' + \
                  ...
                  'Current writing streak is ' + str(trak.core_methods.current_streak()) + ' days'
        else:
            MSG = 'Tracking is OFF\n' + \
                  'Current writing streak is ' + str(trak.core_methods.current_streak()) + ' days'
        print(MSG)
